...
const Classes = () => {
    ...
    const handleSelect = async (selectedClass) => {
        ...
        try {
            ...
            setCartIds(prev => [...prev, selectedClass._id])
            setFeedback(prev => ({ ...prev, [selectedClass._id]: 'Added!' }))
            setTimeout(() => setFeedback(prev => ({ ...prev, [selectedClass._id]: null })), 2000)
        } catch (err) {
            console.log(err);
            setFeedback(prev => ({ ...prev, [selectedClass._id]: err.response?.data?.message || 'Error' }))
            setTimeout(() => setFeedback(prev => ({ ...prev, [selectedClass._id]: null })), 2000)
        }
    };
                        ...
                        >
                            ...
                            <div className="p-4 text-[#712941] flex flex-col flex-1">
                                ...
                                <button
                                    ...
                                    className="mt-auto bg-[#712941] hover:bg-[#c86989] text-white py-1 rounded z-10"
                                >
                                    View
                                </button>
                            </div>
