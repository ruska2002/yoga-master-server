  ...
  return (
    <div className="sm:p-6 font-dancing">
      ...
      {loading ? (
        ...
        <>
          {/* Classes grid */}
          <div className="grid grid-cols-1 min-[535px]:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6 text-[#712941]">
            {selectedClasses.map((cls) => (
              <div key={cls._id} className="shadow-lg rounded p-4 relative">
                ...
                />
                <p className="font-bold text-sm sm:text-base truncate">{cls.name}</p>
                <p className="text-sm">Instructor: {cls.instructorName}</p>
                <p className="text-sm">Price: ${cls.price}</p>
                <button
                  onClick={() => handleRemove(cls._id)}
                  className="absolute top-2 right-2 bg-[#712941] text-white px-2 py-1 rounded hover:bg-[#c86989] text-xs"
                >
                  Remove
                </button>
              </div>
          ...
          {/* Summary */}
          <div className="shadow-lg rounded p-4 w-full sm:w-[60%] md:w-[40%] flex flex-col ml-auto gap-2 text-[#712941] mt-6">
            <h1 className="text-xl font-bold">SUMMARY</h1>
            <hr className="border-[#712941]" />
            ...
            <hr className="border-[#712941]" />
            <div className="flex justify-between mt-2 text-lg sm:text-xl font-bold">
              <p>Total:</p>
              <p>${totalWithTax}</p>
            </div>
            <button
              onClick={goToCheckout}
              className="bg-[#712941] text-white py-3 px-3 rounded-lg hover:bg-[#c86989] text-sm sm:text-base"
            >
              Proceed to Payment
            </button>
          </div>
